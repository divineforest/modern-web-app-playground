...
**Implementation:**
- Define the schema in `src/db/schema.ts` using Drizzle's schema definition API
...
**Database Schema:**
- The `global_contacts` table schema SHALL be defined in `src/db/schema.ts`
...
1. **Define Schema in Drizzle:**
   - Add the `globalContacts` table definition to `src/db/schema.ts`
