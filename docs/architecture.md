...
erDiagram
    %% Core Entities
    companies ||--o{ billing_periods : "has"
    companies ||--o{ jobs : "has"
    companies ||--o{ global_contacts_companies : "relates_to"
    companies ||--o{ invoices : "has"
    users ||--o{ jobs : "assigned_to"

    %% Practice Management
    billing_periods ||--o{ jobs : "contains"
    service_types ||--o{ job_templates : "categorizes"
    service_types ||--o{ jobs : "categorizes"
    job_templates ||--o{ jobs : "templates"

    %% Contact Management
    global_contacts ||--o{ global_contacts_companies : "belongs_to"
    global_contacts ||--o{ global_contacts_documents : "linked_to"
    global_contacts ||--o{ invoices : "appears_in"
    companies ||--o{ global_contacts_companies : "relates_to"

    %% External References
    global_contacts_documents {
        uuid document_id FK "references core.documents"
    }

    %% Legacy Integration
    bob_contacts {
        string bob_id "lookup reference"
    }
    ...
    companies {
        ...
        string billing_inbound_token UK
        string bob_reference_id
    ...
    global_contacts {
        ...
        jsonb postal_address
        jsonb raw_extraction
        jsonb raw_vies_response
        boolean is_valid_vat_id
        timestamptz vat_id_validated_at
        timestamptz created_at
        timestamptz updated_at
    }
